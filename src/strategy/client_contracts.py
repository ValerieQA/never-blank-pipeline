...
def load_stream_contract(path: Path) -> StreamContract:
    ...
    for line in body.splitlines():
        ...
        if match and len(match.group(1)) == 1:
            current = None  # document title: for people, not part of the contract
            continue
        if match and len(match.group(1)) == 2:
            ...
            sections[current] = []
            continue
        if match and current != "Selection":
            raise ClientContractError(
                f"{path}: `{line.strip()}` — sub-headings are allowed only under `## Selection`"
            )
