...
# ===========================================================================
# Independent execution identities (review follow-up)
# ===========================================================================


def test_three_execution_identities_stay_independent_through_the_entrypoint(tmp_path):
    """run_id (A), assignment_id (B), and signal_id (C) all deliberately differ."""
    ...
    # The CLI signal (run-namespace address) carries the true signal identity C,
    # while the assignment identity B comes from the raw signal record
    # (sig-test-001). run_id A is generated by RunContext. All three differ.
    ...
    run_id = decision_path.parent.name  # value A, generated by RunContext
    artifact = DecisionLensDecisionArtifact.model_validate_json(
        decision_path.read_bytes()
    )
    # the persisted, strict-reloaded artifact preserves three exact identities
    assert artifact.run_id == run_id                      # A
    assert artifact.assignment_id == legacy._SIGNAL_ID    # B
    assert artifact.signal_id == distinct_signal          # C
    assert len({artifact.run_id, artifact.assignment_id, artifact.signal_id}) == 3
    # the evaluator received the same three independent identities
    assert len(transport.calls) == 1
