...
- a clause Editorial Revision removed, restored by Instagram → the run stops
  (deterministic resurrection check);
...
def _run(tmp_path, *, composer, judge):
    draft = copy.deepcopy(_draft())
    draft["platforms"]["long"] = {"body": DRAFT, "title": TITLE}
    ...
    with mock.patch.object(sys, "argv", argv), mock.patch.multiple(gap, **patches), \
        ...
        code = main(research_provider=ReadyProvider(), decision_evaluator=evaluator,
                    ...
                    article_revisor=FakeRevisionTransport(FINAL),
...
def test_instagram_restoring_a_clause_revision_removed_fails_closed(tmp_path):
    ...
    # an approving judge: the deterministic resurrection check alone must stop it
    code, patches = _run(tmp_path, composer=composer, judge=RecordingJudge())
    ...
    for attempt in attempts:
        assert "restores content that Editorial Review removed" in attempt["validation_error"]
