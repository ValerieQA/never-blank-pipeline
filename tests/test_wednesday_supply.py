...
# ===========================================================================
# What retrieval still does on the Wednesday route — stated, not implied
# ===========================================================================


def test_the_retrieval_adapter_is_still_constructed_in_production(
    tmp_path, cnbc_feed
):
    """Wednesday still runs evidence RETRIEVAL, and this records that.

    The review asked that Wednesday not instantiate ExaResearchAdapter. It is
    not instantiated for *discovery* — that is now the restored July path, and
    the guard above proves retrieval can never reach the provider's search
    endpoint. But the adapter is still constructed for the other half of what
    it does: fetching the contents of the source the July signal itself cites,
    which is what feeds sources-of-record and what the source-transparency
    gate downstream attests to. Deleting it would publish claims about a
    source nobody verified is reachable — the dead-link defect class again.

    Asserted here so the fact is falsifiable rather than assumed. If the
    product decision is to remove retrieval from Wednesday too, this test is
    the one that should change, deliberately.

    Note the distinction from the harness: every other scenario in this module
    injects `research_provider`, so production's provider selection never
    runs. This one lets it run.
    """
    argv, patches = _entry_patches(tmp_path, dry_run=True)
    argv = ["prog"] + argv[3:] + ["--editorial-role", WEDNESDAY_ROLE_ID]
    del patches["_build_legacy_research_context"]
    patches["generate_for_wednesday"] = mock.MagicMock(
        return_value=_wednesday_article()
    )
    patches["WEDNESDAY_SIGNALS_FILE"] = tmp_path / "wednesday.jsonl"
    # the adapter is stood up offline: this scenario is about WHETHER
    # production constructs it, never about talking to the provider
    ...
    try:
        with mock.patch.object(sys, "argv", argv), mock.patch.multiple(gap, **patches), \
                mock.patch.object(gap, "supply_wednesday_signal",
                                  side_effect=_store_bound_supply(patches)):
            # no research_provider injected: production selects its own
            main(decision_evaluator=_evaluator(_model_output())[0])
    finally:
        for transport in transports:
            transport.stop()

    assert patches["ExaResearchAdapter"].called, (
        "production no longer constructs the retrieval adapter for Wednesday — "
        "if that is intended, Wednesday's evidence chain needs a replacement "
        "source of sources-of-record, not just a removal"
    )
